  ...
  def update
    respond_to do |format|
      if @plan_of_care.update(plan_of_care_params)
        format.html { redirect_to patient_url(@plan_of_care.patient), notice: "Plan of Care was successfully updated for #{@plan_of_care.patient.name}." }
      else
        format.html { redirect_to patient_url(@plan_of_care.patient), alert: "Doh! Plan of Care could not be updated for #{@plan_of_care.patient.name}." }
      end
    end
  end
